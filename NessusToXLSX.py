...
class ReportHostItem:
    def __init__(self, plugin_name=None, plugin_output=None, description=None,cve=None,severity=None,severityID=None):
...
class ReportHost:
    ...
    def print(self):
        ...
        for item in self.reportHostItems:
            item.print()
class Vulnerability:
    def __init__(self, plugin_name, plugin_output, description,cve,severity,severityID,ipList):
        self.ipList = ipList
        self.plugin_name = plugin_name
        self.plugin_output = plugin_output
        self.description = description
...
def ParseReportHostItem(reportHostItem):
    ...
    if plugin_name =="" and plugin_output=="" and description == "" and cve =="":
        return None
    return ReportHostItem(plugin_name,plugin_output,description,cve,GetSeverityString(severityID),int(severityID))
...
def check_input_file(value):
    ...
    return value
def main(input_file, output_file, severity, cve,IpBased):
    ...
    with open(input_file, 'r', encoding='utf-8') as file:
        ...
        if IpBased == False:
            ...
            for vuln in vreport:
                ...
                ws.cell(row=row, column=2, value=vuln.plugin_output)
                ws.cell(row=row, column=3, value=vuln.description)
                ws.cell(row=row, column=4, value=vuln.cve)
                ws.cell(row=row, column=5, value=vuln.severity)
                ws.cell(row=row, column=6, value=', '.join(vuln.ipList))
                row = row + 1
...
if __name__ == "__main__":
    ...
    parser.add_argument("-o", dest="output_file", default="output.xlsx", help="Path to the output file")
    ...
    parser.add_argument("-IpBased", action="store_true", help="IP based output")
    args = parser.parse_args()

    main(args.input_file, args.output_file, args.severity, args.cve,args.IpBased)
